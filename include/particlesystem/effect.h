...
public:
    // Constructor
    Wind(glm::vec2 _position, float _strength)
        : Effects(_position, 5, _strength, {0, 0, 1, 1}) {
...
public:
    // Constructor
    Gravity(glm::vec2 _position, float _gravity)
        : Effects(_position, 10, _gravity, {0, 1, 1, 1}) {
    ...
    void affectParticle(std::vector<Particle>& particle) override {
        ...
        for (size_t i = 0; i < particle.size(); i++) {
            ...
            float distance = (float)sqrt(pow(position.x - particle[i].position.x, 2) +
                                         pow(position.y - particle[i].position.y, 2));
            // vinkeln tas fram mha två sidor på traingeln, för att bara ta diagonalen. Distance =
            // hyp
            float angle = acos((position.x - particle[i].position.x) / distance);
            // Om vinkeln är på negativa sidan flytta till positiva.
            if (position.y < particle[i].position.y) angle *= -1;
